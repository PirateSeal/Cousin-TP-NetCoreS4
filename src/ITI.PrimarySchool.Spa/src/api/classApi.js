...
export async function getClassAssignableListAsync() {
    let classes = await getAsync(endpoint);
    classes.unshift({ name: "", classId: 0 });
    return classes;
}
...
export async function classDetailsAsync(classId) {
    return await getAsync(`${endpoint}/${classId}/Details`);
}
