...
export async function assignTeacherToclassAsync(teacherId, classId) {
    return await postAsync(`${endpoint}/${teacherId}/assignClass`, {
        classId: classId
    });
}

export async function changeAvailabilityAsync(teacherId) {
    return await postAsync(`${endpoint}/${teacherId}/changeAvailability`);
}
