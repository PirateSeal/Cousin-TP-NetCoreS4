  ...
  <div>
    ...
    <form @submit="onSubmit($event)">
      <div class="alert alert-danger" v-if="errors.length > 0">
        ...
        <ul>
          <li v-for="e of errors" :key="e.id">{{e}}</li>
        </ul>
      </div>
      ...
      <div class="form-group">
        ...
        <select v-model="item.classId" class="form-control">
          <option v-for="c of classes" :key="c.classId" :value="c.classId">{{c.name}}</option>
        </select>
      </div>
...
<script>
import { getClassAssignableListAsync } from "../../api/classApi";
import {
  getStudentAsync,
  createStudentAsync,
  updateStudentAsync
...
export default {
  ...
  async mounted() {
    ...
    this.classes = await getClassAssignableListAsync();
