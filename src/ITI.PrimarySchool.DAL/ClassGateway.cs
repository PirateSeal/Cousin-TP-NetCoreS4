    ...
    public class ClassGateway
    {
        ...
        public async Task<Result<int>> Create( string name, string level, int teacherId )
        {
            ...
            if( !IsLevelValid( level ) )
                return Result.Failure<int>( Status.BadRequest, "The class level is not valid." );
            ...
            using( SqlConnection con = new SqlConnection( _connectionString ) )
            {
                ...
                if( status == 1 )
                    return Result.Failure<int>( Status.BadRequest, "A class with this name already exists." );
        ...
        {
            if( !IsNameValid( name ) )
                return Result.Failure<ClassData>( Status.BadRequest, "The class name is not valid." );
            if( !IsLevelValid( level ) )
                return Result.Failure<ClassData>( Status.BadRequest, "The class level is not valid." );
        ...
        public async Task<ClassDetails> GetDetails( int id )
        {
            using( SqlConnection connection = new SqlConnection( _connectionString ) )
            {
                var classFlatDetails =
                    await connection.QueryAsync<ClassFlatDetails>( "Select * from iti.vClassInfo where ClassId = @id",
                        new {id} );

                ClassFlatDetails temp = classFlatDetails.First();
                ...
                ClassDetails classDetails = new ClassDetails
                {
                    ...
                    Teacher = new Teacher
                    {
                        ...
                        TeacherFirstName = temp.TFirstName,
                        TeacherLastName = temp.TLastName
                    }
                };
                foreach( ClassFlatDetails classFlatDetailse in classFlatDetails )
                    students.Add( new Student
                    {
                        StudentId = classFlatDetailse.StudentId,
                        StudentFirstName = classFlatDetailse.SFirstName,
                        StudentLastName = classFlatDetailse.SLastName
                    } );
                return classDetails;
            }
        }
