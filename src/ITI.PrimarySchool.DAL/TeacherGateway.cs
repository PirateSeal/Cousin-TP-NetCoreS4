...
namespace ITI.PrimarySchool.DAL
{
    public class TeacherGateway
    {
        ...
        public async Task<IEnumerable<TeacherData>> GetAll()
        {
            ...
            {
                var teacherDatas = await con.QueryAsync<TeacherData>(
                    @"select t.TeacherId, t.FirstName, t.LastName, t.Availability from iti.vTeacher t;" );
                return teacherDatas;
            }
        }
        ...
        public async Task<Result<int>> Create( string firstName, string lastName )
        {
            ...
            using( SqlConnection con = new SqlConnection( _connectionString ) )
            {
                ...
                if( status == 1 )
                    return Result.Failure<int>( Status.BadRequest, "A teacher with this name already exists." );
        ...
        public async Task<Result> Update( int teacherId, string firstName, string lastName )
        {
            ...
            using( SqlConnection con = new SqlConnection( _connectionString ) )
            {
                ...
                if( status == 1 ) return Result.Failure( Status.NotFound, "Teacher not found." );
                if( status == 2 )
                    return Result.Failure( Status.BadRequest, "A teacher with this name already exists." );
        ...
        public async Task<Result> ChangeAvailability( int id )
        {
            using( SqlConnection connection = new SqlConnection( _connectionString ) )
            {
                ...
                p.Add( "@teacherId", id );
                p.Add( "@Status", dbType: DbType.Int32, direction: ParameterDirection.ReturnValue );
                await connection.ExecuteAsync( "iti.sChangeTeacherAvailability", p,
