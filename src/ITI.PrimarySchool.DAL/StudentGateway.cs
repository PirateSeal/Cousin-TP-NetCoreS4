...
namespace ITI.PrimarySchool.DAL
{
    public class StudentGateway
    {
        ...
        public async Task<Result<int>> Create( string firstName, string lastName, DateTime birthDate,
            string gitHubLogin, int classId )
        {
            if( !IsNameValid( firstName ) )
                return Result.Failure<int>( Status.BadRequest, "The first name is not valid." );
            if( !IsNameValid( lastName ) )
                return Result.Failure<int>( Status.BadRequest, "The last name is not valid." );
            ...
            using( SqlConnection con = new SqlConnection( _connectionString ) )
            {
                ...
                {
                    case 1:
                        return Result.Failure<int>( Status.BadRequest, "A student with this name already exists." );
                    case 2:
                        return Result.Failure<int>( Status.BadRequest, "A student with GitHub login already exists." );
                    default:
        ...
        public async Task<Result> Update( int studentId, string firstName, string lastName, DateTime birthDate,
            string gitHubLogin, int studentClass )
        {
            ...
            using( SqlConnection con = new SqlConnection( _connectionString ) )
            {
                ...
                p.Add( "@BirthDate", birthDate );
                p.Add( "@GitHubLogin", gitHubLogin ?? string.Empty );
                p.Add( "@studentClass", studentClass );
                p.Add( "@Status", dbType: DbType.Int32, direction: ParameterDirection.ReturnValue );
                ...
                if( status == 1 ) return Result.Failure( Status.NotFound, "Student not found." );
                if( status == 2 )
                    return Result.Failure( Status.BadRequest, "A student with this name already exists." );
                if( status == 3 )
                    return Result.Failure( Status.BadRequest, "A student with this GitHub login already exists." );
